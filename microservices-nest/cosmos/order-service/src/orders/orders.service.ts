...
export class OrdersService implements OnModuleInit {
  ...
  private readonly consumer = this.kafka.consumer({ groupId: 'anojaa-order-service' });
  ...
  async create(createOrderDto: createOrderDto): Promise<any> {
    ...
    for (const item of items) {
      ...
      const result = this.redis.set(lockKey, 'locked', 'EX', 3600 * 24, 'NX');

      if (!result) {
        throw new BadRequestException('Unable to acquire lock');
      }
    }
  ...
  async consumeConfirmOrders() {
    await this.consumer.subscribe({ topic: 'anojaa.order.inventory.update' });
